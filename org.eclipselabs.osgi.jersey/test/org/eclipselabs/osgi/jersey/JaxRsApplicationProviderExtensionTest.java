...
import org.eclipselabs.osgi.jersey.resources.TestExtension;
 ...
 * @since 21.09.2017
 */
public class JaxRsApplicationProviderExtensionTest {
	...
	public void testApplicationProviderWithoutResourceProperty() {
		...
		assertFalse(provider.addExtension(new TestExtension(), Collections.emptyMap()));
	}
	...
	public void testApplicationProviderApplicationSelect() {
		...
		extensionProperties.put(JaxRSWhiteboardConstants.JAX_RS_EXTENSION, "true");
		
		assertFalse(provider.addExtension(new TestExtension(), extensionProperties));
		
		// invalid application filter
		extensionProperties.put(JaxRSWhiteboardConstants.JAX_RS_APPLICATION_SELECT, "test");
		assertFalse(provider.addExtension(new TestExtension(), extensionProperties));
		
		// application filter does not match
		extensionProperties.put(JaxRSWhiteboardConstants.JAX_RS_APPLICATION_SELECT, "(name=xy)");
		assertFalse(provider.addExtension(new TestExtension(), extensionProperties));
		
		// application filter matches
		extensionProperties.put(JaxRSWhiteboardConstants.JAX_RS_APPLICATION_SELECT, "(" + JaxRSWhiteboardConstants.JAX_RS_NAME + "=test)");
		assertTrue(provider.addExtension(new TestExtension(), extensionProperties));
	}
